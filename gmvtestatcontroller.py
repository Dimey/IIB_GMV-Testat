...
class TestatController:
    ...
    def connectSignals(self):
        ...
        self.view.BewertungsUebersicht_table.cellClicked.connect(
            self.uebergebeBewertung
        )
        self.view.krit1_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit1_lineEdit, "Kriterium 1")
        )
        self.view.krit2_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit2_lineEdit, "Kriterium 2")
        )
        self.view.krit3_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit3_lineEdit, "Kriterium 3")
        )
        self.view.krit4_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit4_lineEdit, "Kriterium 4")
        )
        self.view.krit5_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit5_lineEdit, "Kriterium 5")
        )
        self.view.krit6_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit6_lineEdit, "Kriterium 6")
        )
        self.view.krit7_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit7_lineEdit, "Kriterium 7")
        )
        self.view.krit8_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit8_lineEdit, "Kriterium 8")
        )
        self.view.krit9_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.krit9_lineEdit, "Kriterium 9")
        )
        self.view.abzug1_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.abzug1_lineEdit, "Abzug 1")
        )
        self.view.abzug2_lineEdit.editingFinished.connect(
            partial(self.speichereKrit, self.view.abzug2_lineEdit, "Abzug 2")
        )
        ...
        self.view.bemerkung_plainTextEdit.textChanged.connect(
            partial(self.speichereBemerkungTextEdit, self.view.bemerkung_plainTextEdit)
        )
    ...
    def oeffneTucanListe(self):
        try:
            ...
            if "tucan" in filename[0].lower():
                ...
                self.view.fuelleLabel(
                    self.view.AnzahlTucanTeilnehmer_label,
                    self.model.tucanliste.shape[0],
                )
    ...
    def oeffneMoodleListe(self):
        try:
            ...
            if "moodle" in filename[0].lower():
                ...
                self.view.fuelleLabel(
                    self.view.AnzahlMoodleTeilnehmer_label,
                    self.model.moodleliste.shape[0],
                )
    ...
    def importiereAlleAbgaben(self):
        ...
        if path:
            ...
            self.importThread._progressSignal.connect(
                self.view.ladeView.updateProgressInfo
            )
            self.importThread._setModelSignal.connect(self.setModelAndViewAfterImport)
            self.importThread.start()
            self.view.fuelleLabel(
                self.view.AnzahlAbgaben_label, len(allePfadeZuAbgaben)
            )
    ...
    def uebergebeBewertung(self, row, column):
        self.geklickteMatrikelnummer = int(
            self.view.BewertungsUebersicht_table.item(row, 0).text()
        )
        self.geklickteZeile = self.model.bewertungsuebersicht.loc[
            self.geklickteMatrikelnummer
        ]

        # Aktiviere Bewertungsdetails
        self.view.aktiviereUIElement(self.view.bewertungsdetails_groupBox, True)
        self.view.aktiviereUIElement(
            self.view.zurAbgabe_btn, self.geklickteZeile["Pfad"] != ""
        )
        self.view.aktiviereUIElement(
            self.view.pdfExport_btn, self.geklickteZeile["Punkte"] != ""
        )
    ...
    def uebergebeStatistik(self):
        ...
        anteilBestanden = (
            ("%.2f" % (anzahlBestandenerAbgaben / anzahlBewertet * 100))
            if anzahlBewertet != 0
            else 0
        )
        gesamtPunktzahl = self.model.gesamtPunktzahl()
        durchschnitt = (
            round(gesamtPunktzahl / anzahlBewertet, 2) if anzahlBewertet != 0 else 0
        )
        self.view.fuelleLabel(self.view.AnzahlAbgaben_label, anzahlAbgaben)
        self.view.fuelleLabel(
            self.view.bestanden_label,
            f"{anzahlBestandenerAbgaben} ({anteilBestanden}%)",
        )
    ...
    def speichereKrit(self, lineEditObj, header):
        self.model.updateBewertungsUebersichtZelle(
            self.geklickteMatrikelnummer, header, lineEditObj.text().replace(",", ".")
        )
        ...
        neueGesamtPunkte = float(
            self.model.bewertungsuebersicht.at[self.geklickteMatrikelnummer, "Punkte"]
        )
        self.view.setzeBewertungsUebersichtZelle(
            row, columnPunkte, f"{neueGesamtPunkte:g}"
        )
        columnBestanden = df.columns.get_loc("Bestanden")
        neuerBestandenStatus = self.model.bewertungsuebersicht.at[
            self.geklickteMatrikelnummer, "Bestanden"
        ]
        self.view.setzeBewertungsUebersichtZelle(
            row, columnBestanden, neuerBestandenStatus
        )
    ...
    def speichereBemerkungTextEdit(self, textEditObj):
        self.model.updateBewertungsUebersichtZelle(
            self.geklickteMatrikelnummer, f"Bemerkungen", textEditObj.toPlainText()
        )
