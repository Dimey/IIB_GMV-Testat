...
class PDFModel(FPDF):
    ...
    def idCheck(cls, kp, werte):
        ...
        F = np.frompyfunc(
            lambda x: f"Augpunkt = (2.75, -2.49, {zKoordinate})" in x, 1, 1
        )(werteStudent)
        E = np.frompyfunc(lambda x: f"SchiebereglerE = {schieberegler}" in x, 1, 1)(
            werteStudent
        )
    ...
    def header(self):
        ...
        self.cell(
            w=0,
            h=8,
            txt="Geometrische Modellierung und Visualisierung I",
            border=False,
            ln=1,
            align="L",
        )
    ...
    def twoPartCell_subTask(self, txt1, pt1, pt2):
        ...
        self.cell(
            w=15.5, h=self.cellHeight, txt=f"{pt2}", align="L", border="TBR", ln=1
        )
    ...
    def grading(self, punkteGesamt, bestehensGrenze):
        ...
        self.cell(
            w=15.5, h=self.cellHeight, txt=f"{punkteGesamt:g}", align="R", border="LT"
        )
        self.set_font(family=self.font, style="", size=self.textHeight)
        ...
        self.cell(
            w=self.cellWidthLeft,
            h=self.cellHeight,
            txt=f"(Bestehensgrenze: {self.bestehensGrenze} Punkte)",
            align="L",
            border="LB",
        )
    ...
    def constructPDF(self, df):
        ...
        self.twoPartCell_subTask(
            "Vielecke zwischen Schatteneckpunkten", df["Kriterium 3"], 4
        )
        ...
        self.twoPartCell_subTask(
            "Eckpunkte Weihnachtsbaum + Stamm", df["Kriterium 5"], 4.5
        )
        self.twoPartCell_subTask(
            "Verbindungslinien Stamm + Krone", df["Kriterium 6"], 3
        )
        ...
        self.twoPartCell_subTask(
            "Eckpunkte Weihnachtsbaum + Stamm", df["Kriterium 8"], 5
        )
        self.twoPartCell_subTask(
            "Verbindungslinien Stamm + Krone", df["Kriterium 9"], 3
        )
        self.emptyLine()
        ...
        self.twoPartCell_penalty(
            "Zahlenwerte an Matrikelnummer nicht korrekt angepasst", df["Abzug 1"]
        )
