...
class PDFModel2(FPDF):
    ...
    def idCheck(cls, kp, werte):
        ...
        F = np.frompyfunc(lambda x: f"Augpunkt = ({xKoordinate}," in x, 1, 1)(
            werteStudent
        )
        G = np.frompyfunc(lambda x: f"SchiebereglerG = {schieberegler}" in x, 1, 1)(
            werteStudent
        )
    ...
    def constructPDF(self, df):
        ...
        self.twoPartCell_subTask(
            "Vielecke zwischen Schatteneckpunkten", df["Kriterium 2"], 2
        )
        ...
        self.twoPartCell_subTask(
            "Eck- und Innenpunkte der Skulptur", df["Kriterium 5"], 4.5
        )
