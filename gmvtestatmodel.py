...
class TestatModel:
    ...
    def erstelleBewertungsUebersichtAusListen(self):
        teilnehmerliste = pd.merge(
            self.moodleliste, self.tucanliste, on=["Nachname", "Vorname"]
        )
        ...
        self.bewertungsuebersicht = teilnehmerliste.set_index(
            "Matrikelnummer", drop=False
        )
    ...
    def speichereBewertungsUebersichtAlsCSV(self):
        ...
        self.bewertungsuebersicht.to_csv(
            f"GMV Testat Tool/Save Files/savefile_{date_time}.csv", index=False
        )

    def ladeBewertungsUebersichtAusCSV(self, pfad):
        self.bewertungsuebersicht = (
            pd.read_csv(pfad).set_index("Matrikelnummer", drop=False).fillna("")
        )
    ...
    def ladeAbgabe(self, path):
        ...
        try:
            ...
            self.bewertungsuebersicht.loc[
                (self.bewertungsuebersicht.Matrikelnummer == int(matrikelnummer)),
                ["Abgabe", "Pfad"],
            ] = ["Ja", ordnerPfad]
            ...
            if anzahlFalscheKriterien == 0:
                ...
                self.bewertungsuebersicht.loc[
                    ...
                    -0,
                    "",
                    "Ein Wert entspricht nicht der Matrikelnummer (kein Abzug).",
                ]
            elif anzahlFalscheKriterien == 2:
                self.bewertungsuebersicht.loc[
                    (self.bewertungsuebersicht.Matrikelnummer == int(matrikelnummer)),
                    ["Abzug 1", "Punkte", "Bemerkungen"],
                ] = [-2, "", "Zwei Werte entsprechen nicht der Matrikelnummer."]
            elif anzahlFalscheKriterien == 3:
                self.bewertungsuebersicht.loc[
                    (self.bewertungsuebersicht.Matrikelnummer == int(matrikelnummer)),
                    ["Abzug 1", "Punkte", "Bemerkungen"],
                ] = [-4, "", "Drei Werte entsprechen nicht der Matrikelnummer."]
            else:
                print(
                    f"ID Check Error! Anzahl der falschen Kriterien: {anzahlFalscheKriterien}."
                )
    ...
    def extrahiereWerteVonZiffern(self, matrikelnummer):
        ...
        return [
            self.variationsMatrix[0][int(E)],
            self.variationsMatrix[1][int(F)],
            self.variationsMatrix[2][int(G)],
        ]
    ...
    def updateBewertungsUebersichtZelle(self, geklickteMatrikelnummer, header, value):
        ...
        self.bewertungsuebersicht.at[geklickteMatrikelnummer, header] = (
            np.NaN if value == "" else value
        )
        ...
        if header != "Bemerkungen":
            ...
            self.bewertungsuebersicht.at[
                geklickteMatrikelnummer, "Punkte"
            ] = gesamtPunktzahl
            self.bewertungsuebersicht.at[geklickteMatrikelnummer, "Bestanden"] = (
                "Ja" if gesamtPunktzahl >= self.bestehensGrenze else "Nein"
            )
    ...
    def updateBestandenStatusAllerStudenten(self):
        ...
        for index_i in df[(df["Abgabe"] == "Ja") & (df["Punkte"] != "")].index:
            bestandenStatus = (
                "Ja" if df.loc[index_i, "Punkte"] >= self.bestehensGrenze else "Nein"
            )
    ...
    def gesamtPunktzahlStudent(self, matrikelnummer):
        return np.clip(
            (
                pd.to_numeric(
                    self.bewertungsuebersicht.loc[
                        matrikelnummer, "Kriterium 1":"Abzug 2"
                    ]
                )
                * self.wertungsSchluessel
            ).sum(),
            a_min=0,
            a_max=30,
        )
    ...
    def exportPDF(self, matrikelNummer):
        ...
        pdf.output(
            f"{pfad}{'/' if pfad != '' else 'GMV Testat Tool/Studenten ohne Abgabe/'}{matrikelNummer}.pdf"
        )
